...
public class ChallengeServiceImpl implements ChallengeService {
    ...
    public Integer createChallenge(ChallengeRequestDTO requestDTO) throws WellnessTrackerException {
        ...
        User manager = optional.orElseThrow(() -> new WellnessTrackerException("Service.USER_NOT_FOUND"));
        ...
        challenge.setIsFeatured(requestDTO.getIsFeatured() != null && requestDTO.getIsFeatured());

        // Resolve status based on start date
        challenge.setStatus(resolveStatus(requestDTO.getStartDate(), requestDTO.getEndDate()));

        // Set department if DEPARTMENT visibility
        if (requestDTO.getDepartmentId() != null) {
            Optional<Department> dept = departmentRepository.findById(requestDTO.getDepartmentId());
            Department department = dept.orElseThrow(() -> new WellnessTrackerException("Service.DEPARTMENT_NOT_FOUND"));
    ...
    public ChallengeResponseDTO getChallengeById(Integer challengeId) throws WellnessTrackerException {
        ...
        Challenge challenge = optional.orElseThrow(() -> new WellnessTrackerException("Service.CHALLENGE_NOT_FOUND"));
        return mapToDTO(challenge);
    }

    // Determines status automatically from dates at creation time
